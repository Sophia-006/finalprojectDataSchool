...
library(tidyverse)
library(Biostrings)
my_database_comp <- mutate(my_database, AAtotal= str_count(sequence, '\\w'), Qcomp= str_count(sequence,'Q')) %>% 
  mutate( Q100=  Qcomp *100 / AAtotal) %>% mutate(Pcomp= str_count(sequence,'P')) %>% 
  mutate(P100= Pcomp *100/AAtotal)
```
```{r, echo=FALSE}
my_database_comp1 <- my_database_comp [,c(1,2,3,5,6,7,8,9)]
write.csv(my_database_comp1, "Data/my_database_comp")
```
```{r, echo=FALSE}
my_database_comp2 <- read.csv('Data/my_database_comp.csv')
knitr::kable(head(my_database, n = 5), format = "html", caption = "Aminoacid composition") %>% 
  kable_styling("striped", full_width = F) %>%
  column_spec(4, width_max = "30em", extra_css="overflow-y: auto") %>% 
  column_spec(2:3, bold=T, color= "blue") %>% 
  footnote(general= "protein.",
          footnote_as_chunk = T)
```
